...
function documentValue() {
    return {
        simulation: {
            schema_version: '2.1',
            meta: { title: 'Script API test', description: 'Exercises Script handlers.', domain: 'qa' },
            world: {
                objects: [
...
const compiled = runtime.compileScript(script);
...
const run = runtime.runProject(documentValue(), script);
assert.equal(run.problems.some(problem => problem.severity === 'error'), false, 'Script project did not execute cleanly');
...
const handles = runtime.compileScript(`
...
assert.throws(() => runtime.compileScript(`
let escaped;
...
const rebound = runtime.compileScript(`
let firstHelper;
...
runtime.compileScript('WorkSpec.task("first").onStart(() => { globalThis.__workspecTestHelper = set; });');
try {
    assert.throws(() => runtime.compileScript('WorkSpec.task("second").onStart(() => globalThis.__workspecTestHelper("worker", "state", "leaked"));'), /only be used inside a task handler/, 'helpers leaked execution context between WorkSpec runs');
} finally {
...
`;
const analysis = runtime.analyzeScript(analysisSource, { taskIds: ['first', 'second'] });
...
assert.equal(analysis.diagnostics.some(diagnostic => diagnostic.code === 'script.task.unknown' && diagnostic.taskId === 'third'), true);
assert.equal(analysis.diagnostics.some(diagnostic => diagnostic.code === 'script.task.dynamic'), true);
const reassignedAnalysis = runtime.analyzeScript('let task = WorkSpec.task("first"); task = chooseTask(); task.onStart(() => {});');
assert.equal(reassignedAnalysis.handlers.length, 0, 'analysis guessed through a reassigned task handle');
assert.equal(reassignedAnalysis.diagnostics.some(diagnostic => diagnostic.code === 'script.task.alias_dynamic'), true);
assert.equal(runtime.analyzeScript('WorkSpec.task("first", task => task.onStart(() => {}));').handlers[0].form, 'grouped', 'expression-bodied grouped form was not indexed');
const unknownRun = runtime.runProject(documentValue(), 'WorkSpec.task("missing").onComplete(() => set("item", "state", "wrong"));');
assert.equal(unknownRun.problems.some(problem => problem.metric_id === 'script.task.unknown'), true, 'runtime did not report a Script handler for an unknown Define task');

// Existing destructured callbacks remain source-compatible, but ambient helpers are canonical.
const legacy = runtime.compileScript('WorkSpec.task("first").onComplete(({ set }) => set("item", "state", "legacy"));');
assert.equal(legacy.get('first:completion').length, 1);

process.stdout.write('✓ WorkSpec 2.1 ambient Script API and analysis\n');
