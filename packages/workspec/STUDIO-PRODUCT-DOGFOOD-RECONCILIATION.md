# WorkSpec 2 Studio product-dogfood reconciliation

Date: 10 September 2026

> Historical report: its unused-resource and validation-orchestration decisions were superseded by the 12 September 2026 WorkSpec 2.2 validation-layer correction. The retained product findings remain useful context.

## 1. Findings reproduced from both runs

- **Canonical timing was the material playback defect.** Studio's local start-time calculation treated dependency-scheduled tasks as if they started at the document clock, truncating the steam-sterilisation story at 07:40. `playground-time-controller.js` now delegates WorkSpec 2 task timing to `WorkSpecRuntime.resolveTimings()`. The flagship scrubber ends at the resolved 10:30 horizon, and all seven Simulation Library templates resolve without timing diagnostics.
...
| `object.optimization.unused_resource` | Historical decision superseded by the validation-layer audit. Current 2.2 document validation makes no utilization claim; project/history validation may report bounded runtime non-use only with an explicit horizon. |
...
| Documentation scope | Keep the targeted WorkSpec 2 authoring guide, navigation entry, terminology corrections, and historical notices. Do not rewrite the historical specification or tutorial corpus as part of the freeze. |
...
- focused WorkSpec 2 documentation and authoring labels.

## 5. Fixes reverted or deliberately rejected

- Historical: the earlier reconciliation reverted blanket suppression. The later validation-layer correction replaced that behavior with scoped document/project validation and an explicit horizon for bounded unused-resource diagnostics.
...
- The five stale steam-template unused-resource suggestions are removed by current 2.2 document validation; bounded project validation sees executed Changes reservations and effects.
